...
Tecnologias **.NET Framework**, **.NET Standard** e **.NET Core** foram utilizadas para o desenvolvimento de componentes de tela/apresentação (View e ViewModel) e também para o desenvolvimento da aplicação final, em WPF (*.NET Framework 4.6.2*, *.NET Core 8.0* e *.NET Core 9.0*).
...
Também foram desenvolvidos instaladores (pt-BR e en-US) para disponibilizar a aplicação para quem se interessar em conhecê-la. Para instaladores, acesse as pastas "installer/net462", "installer/net8.0" ou "installer/net9.0".
...
Após instalar e realizar a geração de um projeto à partir do **CrudGenerator** no *Windows 10*, basta seguir os passos abaixo para compilar e executar o projeto gerado:
 - Método 1:
   ...
   - Passo 2:
     - Opção 1: após finalizar a instalação de **.NET 9.0 SDK** instalado, abra o **Command Prompt do Windows**, navegue até a pasta selecionada para a geração do projeto CRUD via **CrudGenerator** e execute **BuildAndRun.bat**; isto fará com que seja realizado o restore das depenências, compilação e execução do projeto **CRUD**;
